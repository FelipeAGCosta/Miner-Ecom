...
def search_by_category(
    ...
    "fieldgroups": "EXTENDED",  # <--- pede campos extras (inclui estimatedAvailabilities, brand, gtin, etc.)
    }
    ...
    for _ in range(max_pages):
        ...
        for s in summaries:
            item = {
                ...
                # Alguns anúncios trazem disponibilidade estimada neste array:
                # estimatedAvailabilities -> [ { "estimatedAvailableQuantity": N, ... } ]
                "available_qty": None,
                "qty_flag": "EXACT",
                "brand": None,
                "mpn": None,
            ...
            # Tentar extrair quantidade, se presente
            ...
            # Tentar extrair atributos comuns (brand/mpn/gtin) se vierem
            # Em item summary, às vezes aparece "brand" direto; outras vezes, só em ASPECTS (não habilitado por padrão).
            if "brand" in s:
                item["brand"] = s.get("brand")

            # Alguns devolvem "mpn" como parte de "mpn" ou "manufacturerPartNumber" em aspectos.
            if "mpn" in s:
                item["mpn"] = s.get("mpn")

            # GTINs podem vir como "gtin" em alguns casos; se não vier, trataremos depois.
            if "gtin" in s:
...
def get_item_detail(item_id: str) -> dict:
    ...
    # No detalhe, os fieldgroups válidos são: COMPACT, PRODUCT, ADDITIONAL_SELLER_DETAILS, CHARITY_DETAILS
